...
1. Same macOS user (`ahmedmubashir`). Claude desktop → **Code** → open the folder `~/Documents/Invotick/Webinvotick`.
   (Memory is stored on this Mac at `~/.claude/projects/-Users-ahmedmubashir-Documents-Invotick-Webinvotick/memory/`, not in
...
| Memory (incidents, dated state, standing rules) | `~/.claude/projects/-Users-ahmedmubashir-Documents-Invotick-Webinvotick/memory/` — index `MEMORY.md` |
